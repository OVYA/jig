define(
  ...
  function(
     ...
     */
       ...
       */
        var _processResponseReq = function(req, response) {
       ...
       */
        var _processResponse = function(text, xhr) {
          ...
          try {
            ...
            console.error(
              "JiG  API response: invalid JSON string: ",
              text,
              xhr
            );
            for (i in req) {
              if (req.hasOwnProperty(i)) {
                req[i].promise.reject(
                  new ApiError({
      ...
      processException: function(req, response) {
        if (self.showExceptions) {
          value
            .getModule("geonef/jig/tool/dev/ExceptionDump")
            .then(function(_Class) {
              var dump = new _Class(
                lang.mixin(
                  { context: { request: req, response: response } },
                  response.exception
                )
              );
          ...
          window.global.alert(
            "Une erreur est survenue durant la requête serveur.\n" +
