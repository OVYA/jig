...
define(
   ...
   */
      ...
      makeContentNodes: function() {
        return [["div", { _attach: "listNode", class: "results" }]];
      },
   ...
   */
      populateList: function(results) {
        ...
        (results.length > 0 ? domClass.remove : domClass.add)(
          this.domNode,
          "empty"
        );
        var over =
        ...
        this.rows = results.map(this.makeRow, this).map(this.placeRow, this);
        if (over) {
   ...
   */
      ...
      makeRow: function(obj, key) {
        var row = new this.RowClass(
          lang.mixin({ object: obj, listWidget: this }, this.rowOptions)
        );
        return row;
      },
