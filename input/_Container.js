  ...
  function(module, declare, _FormMixin, lang, aspect, widget) {
     ...
     */
      getDescendants: function() {
        var list = [];
        this.getInputRootNodes()
          .map(widget.getFirstNamedDescendants)
        ...
        this.getDescendants()
          ...
          .forEach(function(widget) {
            if (_oldChildrenCnts.hasOwnProperty(widget.id)) {
              ...
              self._childrenCnts[widget.id] = aspect.after(
                widget,
                "onChange",
                lang.hitch(self, self.onChange, widget)
              );
            }
          });
     ...
     */
      getSubWidget: function(name) {
        var child = this.getDescendants().filter(function(ch) {
          return ch.name === name;
        })[0];
        if (!child) {
          console.warn(
            "getSubWidget: child not defined: ",
            name,
            this.getDescendants(),
            this
          );
        ...
        if (!value) {
          ...
          if (this.booleanUnion) {
            descendants.forEach(function(w) {
              w.set("value", value.indexOf(w.get("name")) !== -1);
            });
            ...
            for (i in value) {
              ...
              if (map[i]) {
                map[i].set("value", value[i], !!priorityChange);
     ...
     */
      _getValueAttr: function() {
        var descendants = this.getDescendants(); // this.getChildren(); //getDescendants is deprecated
        var value;
        if (this.booleanUnion) {
          value = descendants
            .filter(function(w) {
          ...
          this.manageValueKeys.forEach(function(p) {
            value[p] = self.get(p);
          });
        }
     ...
     */
      _setReadOnlyAttr: function(state) {
        ...
        if (state === true || state === false) {
          this.getDescendants().forEach(function(input) {
