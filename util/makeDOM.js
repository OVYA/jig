...
define(
  ...
  function(lang, construct, domAttr, _Widget, Tooltip) {
    ...
    function self(args, obj) {
      ...
      if (magic._if !== undefined) {
        ...
        if (magic._if.then) {
          return magic._if.then(function(ret) {
            if (!ret) {
              return null;
            }
      ...
      if (magic._ifNot) {
        if (magic._ifNot.then) {
          return magic._ifNot.then(function(ret) {
            if (ret) {
              return null;
            }
      ...
      if (typeof args[0] === "function") {
        ...
        // assume string - node name of DOMElement to create
        if (magic._NS) {
          node = document.createElementNS(magic._NS, args[0], attrs);
          if (attrs) {
            domAttr.set(node, attrs);
          }
        } else {
