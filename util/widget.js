...
define(
   ...
   */
      getFirstNamedDescendants: function(node) {
        var widgets = [];
        registry.findWidgets(node).forEach(function(w) {
          if (w.name) {
            widgets.push(w);
          } else {
            self.getFirstNamedDescendants(w.domNode).forEach(function(w2) {
              widgets.push(w2);
            });
          }
        });
   ...
   */
      connectOnce: function(obj, event, context, method) {
        kernel.deprecated(
          "geonef/jig/util/widget.connectOnce()",
        ...
        if (obj.addEventListener) {
          if (/^on/.test(event)) {
            ...
            console.warn(
              "widget.connectOnce: obj has an addEventListener method " +
                "but event does not start with 'on'",
              event,
              obj
            );
          }
